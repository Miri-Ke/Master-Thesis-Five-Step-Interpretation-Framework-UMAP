...
"""
Alternative Step 3: Multinomial Logistic Regression
"""
MLR_variable_selection_8clusters = fileD.logregression(dataset = group_means_standardized, label = gmm_labels_8_categorical,
                                                       sort_by = 'f1-score')


MLR_variables = range(len(MLR_variable_selection_8clusters))
MLR_bic_scores = MLR_variable_selection_8clusters['BIC'].values
MLR_f1_scores = MLR_variable_selection_8clusters['f1-score'].values
MLR_variable_labels = MLR_variable_selection_8clusters['Variable'].values  # Store variable names for labeling
...
# Plot BIC scores
color = 'tab:red'
ax1.set_xlabel('Probe Group', fontsize=14)  # Increase font size for x-axis label
ax1.set_ylabel('BIC Score', color=color, fontsize=14)  # Increase font size for y-axis label
ax1.plot(MLR_variables, MLR_bic_scores, color=color, marker='x', label='BIC Score', linewidth=0.5)
ax1.tick_params(axis='y', labelcolor=color, labelsize=12)  # Increase font size for y-axis ticks
ax1.set_xticks(MLR_variables)  # Set x-axis ticks to the new range
ax1.set_xticklabels(MLR_variable_labels, rotation=0, ha="right", fontsize=6)  # Increase font size for x-axis tick labels
ax1.set_ylim([0, max(MLR_bic_scores) * 1.1])  # Set y-axis to start at 0 and extend slightly above max

# Create secondary y-axis for F1 scores
ax2 = ax1.twinx()
color = 'tab:blue'
ax2.set_ylabel('F1 Score', color=color, fontsize=14)  # Increase font size for y-axis label
ax2.plot(MLR_variables, MLR_f1_scores, color=color, marker='o', label='F1 Score', linewidth=1.2)
ax2.tick_params(axis='y', labelcolor=color, labelsize=12)  # Increase font size for y-axis ticks
ax2.set_ylim([0, max(MLR_f1_scores) * 1.1])  # Set y-axis to start at 0 and extend slightly above max

# Add a legend
lines, labels = ax1.get_legend_handles_labels()
lines2, labels2 = ax2.get_legend_handles_labels()
ax2.legend(lines + lines2, labels + labels2, loc='upper left', fontsize=12)  # Increase font size for legend

# Title and layout adjustments
plt.title('F1 and BIC Scores for Logistic Regression Variables Sorted by F1-Score', fontsize=16)  # Increase font size for title
